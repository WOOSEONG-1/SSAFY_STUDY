...
public class Main {
	...
	public static class SegTree {
		...
		// 생성자
		// tree[0] -> 최소값, tree[1] -> 최대값
		SegTree(int[] arr){
			this.N = arr.length;
			this.tree = new int[2][this.N * 4];
		...
		int init_min(int start, int end, int node, int[] arr) {
			...
			int mid = (start + end)/2;
			return tree[0][node] = Math.min(init_min(start, mid, node*2, arr), 
		...
		int init_max(int start, int end, int node, int[] arr) {
			...
			int mid = (start + end)/2;
			return tree[1][node] = Math.max(init_max(start, mid, node*2, arr), 
	...
	public static void main(String[] args) throws IOException {
		...
		for ( int i = 0 ; i < m ; i++ ) {
			...
			int c = Integer.parseInt(stk.nextToken());
				...
				sb.append(sgTree.find_max(0, n-1, 1, b-1, c-1)).append("\n");
				arr[b-1] = c;		
		}
